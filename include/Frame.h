...
//          ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth);
    ...
    void ExtractORBDesp();
    bool CreateMask(const cv::Mat &imS, const cv::Mat &imD, cv::Mat &Mask, std::vector<int> &cols_seg);
    void RemoveKpt(const cv::Mat &Mask, const std::vector<int> cols_seg);
    // 各层的特征点
    std::vector<std::vector<cv::KeyPoint>> mvAllKeys;
    bool bCreateOK;
    cv::Mat imFusion;
    cv::Mat Mask;
