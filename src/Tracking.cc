...
void Tracking::SetSegment(Segment *segment) { mpSegment = segment; }
...
void Tracking::GetImg(const cv::Mat &img) {
  unique_lock<mutex> lock_NewImg(mpSegment->mMutexGetNewImg);
  mpSegment->mbNewImgFlag = true;
  mpSegment->mImg = img.clone();
  mpSegment->mCondGetNewImg.notify_all();
}
// NOTE: 检测语义图像是否到达
void Tracking::WaitImgS() {
  unique_lock<mutex> lock_NewImS(mpSegment->mMutexSegOK);
  while (!mpSegment->mbSegOK) {
    mpSegment->mCondSegOK.wait(lock_NewImS);
  }
}
// NOTE: 请求语义图像
void Tracking::RequestImgS() {
  unique_lock<mutex> lock_Request(mpSegment->mMutexReqImgS);
  mpSegment->mbReqImgS = true;
}

// NOTE: 获取语义图像
void Tracking::GetImgS() {
  unique_lock<mutex> lock_NewImS(mpSegment->mMutexSegOK);
  mImS = mpSegment->mImgSegmentLatest.clone();
  //    cv::imwrite("ImgS.png",mImS);
}
// NOTE:检测分割是否完成
bool Tracking::CheckSegOK() {
  unique_lock<mutex> lock_SegOK(mpSegment->mMutexSegOK);
  //    cout<<"Tracking Check:the SegOK is"<<(int)mpSegment->mbSegOK<<endl;
  return mpSegment->mbSegOK;
}
// 简化版的跟踪
bool Tracking::TrackForSeg() {
...
void Tracking::Track() {
  ...
  if (mState == NOT_INITIALIZED) {
    if (mSensor == System::STEREO || mSensor == System::RGBD)
      StereoInitialization(); // 双目和RGBD相机的初始化
    else
    ...
    //                }
    //                GetImgS();
    //                Frame mKeyFrameTemp(mCurrentFrame);
    //                // 重新构造当前帧
    //                std::chrono::steady_clock::time_point t1 =
    //                std::chrono::steady_clock::now();
    //                mCurrentFrame = Frame(mImGray, mImDepth, mImUsDepth, mImS,
    //                mTimeStamp, mpORBextractorLeft, mpORBVocabulary, mK,
    ...
    //                - t1).count();
    //                mvTimeORB.push_back(ExtractORBTime);
    //                if(mCurrentFrame.bCreateOK)
    //                {
    //                    bOK = TrackForSeg();
    ////                    cout<<"重新跟踪是否成功"<<bOK<<endl;
    //                }
    //                else {
    //                    mCurrentFrame = Frame(mKeyFrameTemp);
    //                }
    //                if(bOK)
    //                    bKeyFrame = true;
    //            }
...
void Tracking::StereoInitialization() {
  if (mCurrentFrame.N > 500) {
    ...
    //        if(mCurrentFrame.RemoveKpt(mImS, mImUsDepth))
    //        {
    //            mCurrentFrame.ExtractORBDesp();
    //        }
